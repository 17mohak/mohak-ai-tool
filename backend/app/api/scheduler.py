...
async def update_department(dept_id: int, body: DeptIn, db: AsyncSession = Depends(get_db)):
...
@router.get("/batches")
async def list_batches(department_id: int | None = None, db: AsyncSession = Depends(get_db)):
    q = select(Batch).order_by(Batch.department_id, Batch.parent_batch_id.is_(None).desc(), Batch.name)
...
async def update_batch(batch_id: int, body: BatchIn, db: AsyncSession = Depends(get_db)):
...
@router.get("/teachers")
async def list_teachers(department_id: int | None = None, db: AsyncSession = Depends(get_db)):
...
async def update_teacher(teacher_id: int, body: TeacherIn, db: AsyncSession = Depends(get_db)):
...
async def create_unavailability(body: UnavailabilityIn, db: AsyncSession = Depends(get_db)):
    ...
    return [
        {
            ...
            "batch_id": s.batch_id,
            "batch_name": batches_map.get(s.batch_id, ""),
...
async def update_subject(subject_id: int, body: SubjectIn, db: AsyncSession = Depends(get_db)):
...
async def validate_department(dept_id: int, db: AsyncSession = Depends(get_db)):
    ...
    if not batches:
        errors.append({"type": "no_batches", "message": f"No batches found for {dept.name}."})

    # Teachers
    tr = await db.execute(select(Teacher).where(Teacher.department_id == dept_id))
    teachers = tr.scalars().all()
    if not teachers:
        errors.append({"type": "no_teachers", "message": f"No teachers found for {dept.name}."})

    # Subjects
    sr = await db.execute(select(Subject).where(Subject.department_id == dept_id))
    subjects = sr.scalars().all()
    if not subjects:
        errors.append({"type": "no_subjects", "message": f"No subjects found for {dept.name}."})
...
@router.get("/runs")
async def list_runs(department_id: int | None = None, db: AsyncSession = Depends(get_db)):
    q = (
        select(TimetableRun)
...
async def get_department_state(dept_name: str, db: AsyncSession = Depends(get_db)):
    """Return all config data for a department in one call."""
    dept = await _resolve_dept(db, dept_name)
    did = dept.id

    br = await db.execute(select(Batch).where(Batch.department_id == did).order_by(Batch.name))
    batches = br.scalars().all()

    tr = await db.execute(select(Teacher).where(Teacher.department_id == did).order_by(Teacher.name))
    teachers = tr.scalars().all()

    sr = await db.execute(select(Subject).where(Subject.department_id == did).order_by(Subject.name))
    subjects = sr.scalars().all()
    ...
    pr = await db.execute(
        select(PinnedSlot)
        .options(joinedload(PinnedSlot.subject))
        .where(PinnedSlot.subject_id.in_([s.id for s in subjects]) if subjects else PinnedSlot.id < 0)
    )
    pins = pr.scalars().all()
...
async def generate_variants(dept_name: str, db: AsyncSession = Depends(get_db)):
    ...
    for i in range(3):
        label = f"V{i+1}"
        await broadcast.broadcast({
            "type": "task_log", "agent_id": 999, "agent_name": "Timetable AI",
            "task_description": f"Generating {label} for {dept.name}",
            "task_status": "Running",
        })
        res = await generate_schedule(db, dept.id)
        res["variant"] = label
        results.append(res)
...
async def get_run_slots(run_id: int, batch_id: int | None = None, db: AsyncSession = Depends(get_db)):
