...
const filePath = path.join('c:', 'Users', 'MOHAK', 'mohak-ai-tool', 'frontend', 'src', 'app', '(dashboard)', 'scheduler', 'page.tsx');
let content = fs.readFileSync(filePath, 'utf8');

// 1. Add batchStructureOpen state at line 166
content = content.replace(
  `  /* ── Pinned slots ── */\r\n  const [pinsOpen, setPinsOpen] = useState(false);`,
  `  /* ── Pinned slots ── */\r\n  const [pinsOpen, setPinsOpen] = useState(false);\r\n  const [batchStructureOpen, setBatchStructureOpen] = useState(false);`
);

// Another variant just in case line-endings are different:
content = content.replace(
  `  /* ── Pinned slots ── */\n  const [pinsOpen, setPinsOpen] = useState(false);`,
  `  /* ── Pinned slots ── */\n  const [pinsOpen, setPinsOpen] = useState(false);\n  const [batchStructureOpen, setBatchStructureOpen] = useState(false);`
);

// 2. Extract out what we're going to replace.
// We are replacing from `  return (\n    <div className="flex gap-6 h-[calc(100vh-5rem)]">` to the end of the file.
const returnMarker = '  return (\n    <div className="flex gap-6 h-[calc(100vh-5rem)]">';
const returnMarkerCrLf = '  return (\r\n    <div className="flex gap-6 h-[calc(100vh-5rem)]">';

let markerIndex = content.indexOf(returnMarker);
if (markerIndex === -1) markerIndex = content.indexOf(returnMarkerCrLf);
...
if (markerIndex !== -1) {
  const newReturnBlock = \`  return (
    <div className="flex flex-col h-[calc(100vh-5rem)] relative overflow-hidden bg-slate-950">
      ...
      <div className="flex flex-1 gap-4 overflow-hidden p-3 pb-0">
        ...
        <div className="w-[30%] min-w-[320px] max-w-[380px] flex flex-col gap-3 shrink-0 h-full">
                  ...
                  <div className="mt-2 space-y-1.5 pt-2 border-t border-slate-700/50">
                    ...
                    <div className="flex gap-1.5">
                      ...
                      <select value={newPinSlot} onChange={e => setNewPinSlot(Number(e.target.value))}
                        className="flex-1 w-1/2 px-2 py-1.5 bg-slate-800 border border-slate-600 rounded text-[10px] text-slate-200 outline-none focus:ring-1 focus:ring-indigo-500/50">
                        {TIME_SLOTS.map(s => <option key={s.index} value={s.index}>S{s.index}</option>)}
                      </select>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="flex gap-1.5 overflow-hidden justify-end">
                        ...
                        <span className="text-[9px] text-amber-500 font-bold bg-amber-500/10 px-1.5 py-0.5 rounded whitespace-nowrap border border-amber-500/20 shrink-0">
                          {s.credits - gridSlots.filter(g => g.subject_id === s.id).length} lef
                        </span>
                      </span>
                    </div>
                  </div>
          ...
          {error && <div className="p-2 border border-red-500/30 bg-red-500/10 rounded text-[10px] text-red-300 leading-tight font-medium shadow-sm">{error}</div>}
        </div>
            ...
            <div className="flex-1 overflow-auto custom-scrollbar bg-slate-950/20">
              <table className="w-full h-full min-w-[700px] border-collapse relative">
                ...
                <tbody>
                  {TIME_SLOTS.map(ts => (
                    <tr key={ts.index}>
                      <td className="p-2 border-b border-r border-slate-700/60 bg-slate-800/60 align-middle text-center w-20 relative hover:bg-slate-800 transition-colors">
                      ...
                      {DAYS.map(day => {
                        ...
                        let cellBgClasses = "bg-transparent transition-all duration-200 ";
                        ...
                        return (
                          ...
                          >
                            {entries.length > 0 && (
                              <div className="absolute inset-1.5 flex flex-col gap-1.5 overflow-y-auto custom-scrollbar pr-0.5">
                                {entries.map(e => {
                                  const pinned = isPinnedSlot(e.subject_id, day, ts.index);
                                  const dailyLoad = teacherLoadMap.get(\`\${day}-\${e.teacher_id}\`) || 0;
                                  const maxLoad = teacherLimits.get(e.teacher_id) || 4;
                                  const isOverloaded = dailyLoad > maxLoad;
                                  ...
                                  return (
                                    ...
                                    >
                                      <div className={\`absolute top-0 left-0 w-0.5 h-full \${e.is_lab ? "bg-purple-500/50" : "bg-indigo-500/50"}\`}></div>
                                      <div className="flex items-start justify-between mb-1 min-w-0">
                                        <p className="font-bold text-[11px] text-slate-200 leading-[1.15] break-words pr-1">{e.subject}</p>
                                        {pinned && <span className="text-[10px] text-amber-500 shrink-0">📌</span>}
                                      </div>
                                      <p className="text-[10px] text-slate-400 font-semibold truncate group-hover:text-slate-300 transition-colors">{e.teacher}</p>
                                      <div className="flex justify-between mt-1 pt-1 border-t border-slate-600/30 text-[9px] text-slate-500 font-bold uppercase tracking-wider">
                                        <span className="truncate max-w-[50%] opacity-80">{e.batch}</span>
                                        <span className="truncate opacity-80">{e.room}</span>
                                      </div>
                                    </motion.div>
                                  );
                                })}
  ...
  content = content.substring(0, markerIndex) + newReturnBlock;
  fs.writeFileSync(filePath, content, 'utf8');
  console.log("Replaced successfully!");
} else {
  console.log("Marker not found, could not replace block.");
}
